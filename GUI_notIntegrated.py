...
class GraphPage(QtWidgets.QWidget):
    ...
    def load_csv(self, file_path):
        try:
            df = pd.read_csv(file_path); df['Time'] = pd.to_datetime(df['Time'])
            ...
            self.ax.plot(df_filtered['Relative_Hour'], df_filtered['Temperature'], color='#AAFF7F', linewidth=2, marker='o', markersize=3)
            self.ax.set_title("Temperature Reading Log (Latest 24H)", color='#AAFF7F', fontweight='bold', fontsize=14)
            self.ax.set_xlabel("Time (Hours)"); self.ax.set_ylabel("Degrees (°C)")
            self.ax.set_xlim(0, 24); self.ax.set_xticks(range(0, 25, 4)); self.ax.set_ylim(16, 45)
...
class ScalingStack(QtWidgets.QStackedWidget):
    def resizeEvent(self, event):
        ...
        for i in range(self.count()):
            ...
            # --- SCALE DASHBOARD & ALL WIDGETS ---
            for child in page.findChildren(QtWidgets.QWidget):
                # Position/Size scaling (only if they aren't inside a managed layout, but Dashboard uses geoms)
                if page.__class__.__name__ == "DashboardWidget":
                    if hasattr(child, '_orig_geo'):
                        geo = child._orig_geo
                    else:
                        geo = child.geometry()
                        child._orig_geo = geo
                ...
                # Font scaling via Stylesheet
                if hasattr(child, '_orig_style'):
                    style = child._orig_style
                else:
            ...
            # --- SPECIAL HANDLING FOR PEAK PAGE (PAGE 3) ---
            if isinstance(page, PeakSchedulerPage):
                # Scale the table font
                f = page.table.font()
                f.setPointSize(int(12 * avg_scale)) # Base size 12
                page.table.setFont(f)
                # Scale the header font
                h_f = page.table.horizontalHeader().font()
                h_f.setPointSize(int(11 * avg_scale))
                page.table.horizontalHeader().setFont(h_f)
...
class ThermostatApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.min_temp_c, self.max_temp_c, self.set_temp_c, self.current_temp_c = 16, 45, 24, 20.0
        ...
        self.current_data_file = "temp_updated.csv"; self.pages.currentChanged.connect(self.on_page_switched)
    ...
    def on_page_switched(self, index):
        if index == 1: self.page2.load_csv(self.current_data_file)
