...
def tes_ahu_simple(T_amb, T_des, T_tank, peak_state):
    T_full, T_low = 60.0, 40.0
    need_heat = T_amb < T_des
    tank_low, tank_full = T_tank <= T_low, T_tank >= T_full
    ahu_state, tes_state, case_id = AHUState.IDLE, TESState.IDLE, 0

    if need_heat:
        if peak_state == 1:
            if not tank_low: ahu_state, tes_state, case_id = AHUState.VENT, TESState.DISCHARGE, 1
            else: ahu_state, tes_state, case_id = AHUState.NORMAL, TESState.IDLE, 2
        else:
            if not tank_full: ahu_state, tes_state, case_id = AHUState.NORMAL, TESState.CHARGING, 3
            else: ahu_state, tes_state, case_id = AHUState.NORMAL, TESState.IDLE, 4
    else:
        if peak_state == 1: ahu_state, tes_state, case_id = AHUState.IDLE, TESState.IDLE, 5
        else:
            if not tank_full: ahu_state, tes_state, case_id = AHUState.IDLE, TESState.CHARGING, 6
            else: ahu_state, tes_state, case_id = AHUState.IDLE, TESState.IDLE, 7
...
class PeakSchedulerPage(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.title = QtWidgets.QLabel("WEEKLY PEAK HOUR SETTINGS"); self.title.setStyleSheet("color: #AAFF7F; font-size: 22px; font-weight: bold; margin-bottom: 15px;"); self.title.setAlignment(QtCore.Qt.AlignCenter); self.layout.addWidget(self.title)
...
class SmartSchedulePage(QtWidgets.QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)

        self.setStyleSheet("background-color: #0F141C;")
        self.selected_day = "Mon"
        self.days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]

        self.schedule = {
            day: [
                {"mode": "Home", "start": "07:00", "end": "08:30", "temp": 75},
                {"mode": "Away", "start": "08:30", "end": "16:30", "temp": 76},
                {"mode": "Home", "start": "16:30", "end": "23:00", "temp": 75},
                {"mode": "Sleep", "start": "23:00", "end": "07:00", "temp": 74},
            ]
        ...
        title = QtWidgets.QLabel("SMART SCHEDULE")
        ...
        self.card_area = QtWidgets.QVBoxLayout()
        self.card_area.setSpacing(22)

        main.addLayout(self.card_area, stretch=1)
    ...
    def refresh_cards(self):
        ...
        for i, block in enumerate(self.schedule[self.selected_day]):
            ...
            layout.setContentsMargins(45, 20, 30, 20)
            layout.setSpacing(18)

            mode = QtWidgets.QLabel(block["mode"])
            mode.setMinimumWidth(600)
            mode.setAlignment(QtCore.Qt.AlignVCenter | QtCore.Qt.AlignLeft)
            mode.setStyleSheet("""
                color: white;
                font-size: 60px;
                font-weight: bold;
            ...
            start = QtWidgets.QTimeEdit(QtCore.QTime.fromString(block["start"], "HH:mm"))
            end = QtWidgets.QTimeEdit(QtCore.QTime.fromString(block["end"], "HH:mm"))

            for t in (start, end):
                t.setDisplayFormat("hh:mm AP")
                t.setMinimumHeight(85)
            ...
            start.timeChanged.connect(lambda val, idx=i: self.update_time(idx, val, True))
            end.timeChanged.connect(lambda val, idx=i: self.update_time(idx, val, False))

            temp = QtWidgets.QSpinBox()
            temp.setRange(60, 85)
            temp.setValue(block["temp"])
            temp.setSuffix("°F")
            temp.setMinimumHeight(90)
            temp.setMinimumWidth(170)
            temp.setStyleSheet("""
                QSpinBox {
                    background-color: #2D3848;
                    color: #AAFF7F;
            ...
            """)
            temp.valueChanged.connect(lambda val, idx=i: self.update_temp(idx, val))
            ...
            self.card_area.addWidget(card, stretch=1)

    def update_temp(self, index, value):
        self.schedule[self.selected_day][index]["temp"] = value

    def update_time(self, index, qtime, is_start):
        key = "start" if is_start else "end"
        self.schedule[self.selected_day][index][key] = qtime.toString("HH:mm")
...
class GraphPage(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.x_data, self.y_enc, self.y_tank, self.y_desired = deque(maxlen=50), deque(maxlen=50), deque(maxlen=50), deque(maxlen=50)
    ...
    def apply_style(self):
        ...
        self.ax.tick_params(colors='white', labelsize=10); self.ax.xaxis.label.set_color('#7a869a'); self.ax.yaxis.label.set_color('#7a869a')
    def update_live_data(self, enc_temp, tank_temp, desired_temp):
        self.x_data.append(time.time() - self.start_time)
        self.y_enc.append(enc_temp); self.y_tank.append(tank_temp); self.y_desired.append(desired_temp)
...
class ScalingStack(QtWidgets.QStackedWidget):
    def resizeEvent(self, event):
        ...
        for i in range(self.count()):
            ...
            # Do NOT auto-scale the Smart Schedule page
            if isinstance(page, SmartSchedulePage):
                continue

            for c in page.findChildren(QtWidgets.QWidget):
                if page.__class__.__name__ == "DashboardWidget":
                    if not hasattr(c, '_orig_geo'):
                        c._orig_geo = c.geometry()
                    c.setGeometry(
                        int(c._orig_geo.x() * sx),
                        int(c._orig_geo.y() * sy),
                        int(c._orig_geo.width() * sx),
                        int(c._orig_geo.height() * sy)
                    )

                if not hasattr(c, '_orig_style'):
                    c._orig_style = c.styleSheet()

                if c._orig_style and "font-size:" in c._orig_style:
                    c.setStyleSheet(
                        re.sub(
                            r'font-size:\s*(\d+)px',
                            lambda m: f'font-size: {int(int(m.group(1)) * avg)}px',
                            c._orig_style
                        )
                    )
            ...
            if isinstance(page, PeakSchedulerPage):
                f = page.table.font()
                f.setPointSize(int(12 * avg))
                page.table.setFont(f)

                hf = page.table.horizontalHeader().font()
                hf.setPointSize(int(11 * avg))
                page.table.horizontalHeader().setFont(hf)
...
class ThermostatApp(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.page3 = PeakSchedulerPage(self)
        self.page4 = SmartSchedulePage(self)
    ...
    def global_update(self):
        ...
        # 2. FSM CYCLE (Serial Printing Restored)
        ...
        # --- RESTORED SERIAL OUTPUTS ---
        print('\n================================================')
        print(f'SYSTEM STATUS - {datetime.now().strftime("%I:%M:%S %p")}')
        print(f'Mode: {"PEAK" if self.peak_state else "OFF-PEAK"}')
        print(f'T_amb: {self.current_temp_c:.2f} °C | T_tank: {self.tank_temp_c:.2f} °C | T_des: {self.set_temp_c:.2f} °C')
        print('--- FSM STATES ---')
        print(f'AHU State: {ahu.name} | TES State: {tes.name} | Case ID: {cid}')
        print('--- COMPONENT STATUS ---')
        print(f'Fan: {"ON" if b else "OFF"} | Pump: {"ON" if p else "OFF"} | Heater: {"ON" if h else "OFF"}')
        print('================================================\n')
