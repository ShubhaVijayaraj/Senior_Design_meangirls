...
class GraphPage(QtWidgets.QWidget):
    ...
    def load_log(self, file_path, is_celsius=True):
        try:
            ...
            df_filtered['Relative_Hour'] = (df_filtered['timestamp'] - start_time).dt.total_seconds() / 3600

            self.ax.clear()
            self.apply_style()

            y_data = df_filtered['T_amb_C']
            y_label = 'Ambient Temperature (°C)'
            y_min, y_max = 16, 30

            if not is_celsius:
                y_data = (df_filtered['T_amb_C'] * 9 / 5) + 32
                y_label = 'Ambient Temperature (°F)'
                y_min, y_max = 60.8, 86

            self.ax.plot(df_filtered['Relative_Hour'], y_data, color='#AAFF7F', linewidth=2, marker='o', markersize=3)
            self.ax.set_title('Ambient Temperature Log (Latest 24H)', color='#AAFF7F', fontweight='bold', fontsize=14)
