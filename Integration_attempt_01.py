...
# =========================================================
# SHARED VARIABLES BETWEEN GUI AND FSM
# =========================================================
desired_temp_from_gui = 22.0
actual_temp_from_fsm = 22.0
...
# 9. MAIN LOOP
def main():
    global desired_temp_from_gui, actual_temp_from_fsm
    ...
    try:
        while True:
            ...
            thermostat_temp = read_airtemp(1)   # room / thermostat temp
            fan_inlet_temp = read_airtemp(2)    # inlet of fan temp
            ...
            # DESIRED TEMP NOW COMES FROM GUI
            T_des = desired_temp_from_gui
            peak_state = read_peak_state()

            # send actual temp to GUI
            actual_temp_from_fsm = T_amb
...
# ================= BACKGROUND LABELS ======
bg_img = Image.open("cool_to_warm_gradient.png")
...
def update_target_display():
    global desired_temp_from_gui

    temp = target_temp.get()
    desired_temp_from_gui = temp
...
def update_target(val=None):
    ...
    desired_temp_from_gui = target_temp.get()
...
def toggle_fan():
    # USER REQUEST: fan button should do nothing
    pass
...
def sync_actual_temp_from_fsm():
    global actual_temp_from_fsm
    current_temp.set(actual_temp_from_fsm)
    draw_dial(current_temp.get())
    root.after(1000, sync_actual_temp_from_fsm)
...
# ================= START =================
def start_fsm_background():
    fsm_thread = threading.Thread(target=main, daemon=True)
    fsm_thread.start()

page_main.tkraise()
update_inputs("Idle")
sync_actual_temp_from_fsm()
start_fsm_background()
root.mainloop()